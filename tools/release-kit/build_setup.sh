...
main() {
  ...
  # Clone pil2-compiler
  ensure git clone https://github.com/0xPolygonHermez/pil2-compiler.git
  cd pil2-compiler
  ...
  if [[ -n "$PIL2_COMPILER_BRANCH" ]]; then
    ...
    ensure git checkout "$PIL2_COMPILER_BRANCH"
  fi
  ...
  ensure git clone https://github.com/0xPolygonHermez/pil2-proofman-js.git
  cd pil2-proofman-js
  ...
  if [[ -n "$PIL2_PROOFMAN_JS_BRANCH" ]]; then
    ...
    ensure git checkout "$PIL2_PROOFMAN_JS_BRANCH"
  fi
  rm -rf package-lock.json
  rm -rf node_modules
  cd ..
  ...
  step  "Compiling ZisK PIL..."
      ...
      -I pil,../pil2-proofman/pil2-components/lib/std/pil,state-machines,precompiles -o pil/zisk.pilout

  step  "Generate fixed data..."
  ensure cargo run --release --bin keccakf_fixed_gen
  mkdir -p build
  ensure mv precompiles/keccakf/src/keccakf_fixed.bin build

  step  "Generate setup data..."
  ensure node --max-old-space-size=65536 ../pil2-proofman-js/src/main_setup.js \
      -a ./pil/zisk.pilout -b build -i ./build/keccakf_fixed.bin -r

  step "Copy provingKey directory to \$HOME/.zisk directory..."
  ensure cp -R build/provingKey "$HOME/.zisk"
